...
from novel_assistant.ui.pages import character_page
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="wensha_character_", dir=str(ROOT)) as temp_dir:
        ...
        target_group_item = next(
            (
                window.character_tree.topLevelItem(index)
                for index in range(window.character_tree.topLevelItemCount())
                if (window.character_tree.topLevelItem(index).data(0, Qt.UserRole) or {}).get("name") == "旧港商会"
            ),
            None,
        )
        assert_true(target_group_item is not None, "应能找到用于新增人物的目标分组")
        original_card_id = window.current_character_id
        window.character_tree.setCurrentItem(target_group_item)
        app.processEvents()
        original_get_text = character_page.QInputDialog.getText
        try:
            character_page.QInputDialog.getText = staticmethod(lambda *args, **kwargs: ("旧港测试人物", True))
            add_character_button = find_button_by_text(window, "+ 人物")
            assert_true(add_character_button is not None, "应能找到新增人物按钮")
            add_character_button.click()
            app.processEvents()
        finally:
            character_page.QInputDialog.getText = original_get_text
        added_card = next((card for card in window.ensure_character_data()["cards"] if card.get("name") == "旧港测试人物"), None)
        assert_true(added_card is not None, "点击新增人物后应创建人物卡")
        assert_true(added_card.get("faction") == "旧港商会", "新增人物应归入当前选中的分组，而不是第一个分组")
        window.populate_character_tree(original_card_id)
        window.load_character(original_card_id)
