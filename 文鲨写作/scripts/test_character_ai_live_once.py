...
from novel_assistant.main import DraftStore, ProjectHomeWindow, ProjectMeta, default_character_ai_scope, load_application_fonts  # noqa: E402
from test_character_page import CURRENT_BODY_MARKER, OTHER_BODY_MARKER, PRIVATE_WORLD_MARKER, build_character_ai_scope_draft  # noqa: E402
...
def main() -> int:
    ...
    original_start = window.start_character_ai_stream
    ...
    window.start_character_ai_stream = MethodType(capture_and_start, window)  # type: ignore[method-assign]

    with tempfile.TemporaryDirectory(prefix="wensha_character_ai_live_", dir=str(ROOT)) as temp_dir:
        project = ProjectMeta(name="AI人物实测", path=temp_dir, auto_save_minutes=10)
        ...
        window.ai_role_prompt_edit.setPlainText("输出简洁，优先检查人物动机与关系。")
        ...
        window.character_ai_input.setText("请只用一句中文短句说明：人物AI建议正常。")
        window.send_character_ai_message()
        deadline = time.time() + 70
        while window.character_ai_thread is not None and time.time() < deadline:
            app.processEvents()
            time.sleep(0.05)
        app.processEvents()

        if window.character_ai_thread is not None:
            window.stop_character_ai_stream()
            return fail("stream_timeout")
        if not window.character_ai_stream_text.strip():
        ...
        if "CHARACTER_CURRENT_ALLOWED" not in payload or "CHARACTER_SUMMARY_CURRENT" not in payload:
            return fail("scope_missing_expected_context")
        print("stream_ok")
        print(f"stream_chars={len(window.character_ai_stream_text.strip())}")
